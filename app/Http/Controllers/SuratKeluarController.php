...
class SuratKeluarController extends Controller
{
    ...
    {
        $query = SuratKeluar::with(['jenisSurat', 'user']);
        ...
        return view('surat-keluar.index', compact('daftarSurat', 'daftarJenis'));
    }
    ...
    public function simpan(Request $permintaan)
    {
        ...
        if ($permintaan->hasFile('file_surat')) {
            $path = $permintaan->file('file_surat')->store('surat-keluar', 'public');
            $validasi['file_surat'] = $path;
        }

        $validasi['id_user_input'] = Auth::id();

        SuratKeluar::create($validasi);

        if ($permintaan->ajax()) {
            return response()->json(['success' => true, 'message' => 'Surat keluar berhasil ditambahkan.']);
        }
        return redirect()->route('surat-keluar.index')->with('sukses', 'Surat keluar berhasil ditambahkan.');
    }
    ...
    public function ubah(Request $permintaan, $id)
    {
        ...
        if ($permintaan->ajax()) {
            return response()->json(['success' => true, 'message' => 'Surat keluar berhasil diperbarui.']);
        }
        return redirect()->route('surat-keluar.index')->with('sukses', 'Surat keluar berhasil diperbarui.');
    }
    ...
    public function hapus($id)
    {
        ...
        return redirect()->route('surat-keluar.index')->with('sukses', 'Surat keluar berhasil dihapus.');
    }
}
